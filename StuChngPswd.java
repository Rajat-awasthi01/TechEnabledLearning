 ...
 * @author chandan
 */
public class StuChngPswd extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            hans = request.getParameter("hint_answr");
            cpswd = request.getParameter("cpswd");
            npswd = request.getParameter("npswd");

            DbConnection db = new DbConnection();
            db.rst = db.stmt.executeQuery("select * from stu_login where enrol_no='" + enroll + "' and  pswd='" + cpswd + "' and hint_answr='" + hans + "'");
            if (db.rst.next()) {
                db.pstmt=db.con.prepareStatement("UPDATE stu_login SET  pswd='"+npswd+"'  WHERE enrol_no = '"+enroll+"'");
